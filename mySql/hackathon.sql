...
CREATE TABLE IF NOT EXISTS `hackathon`.`resource` (
  ...
  `age` ENUM('child', 'young adult', 'middle-aged adults', 'old-aged adults') NULL DEFAULT NULL,
  `sex` ENUM('male', 'female') NULL DEFAULT NULL,
  `imageCategory` ENUM('face', 'full-body') NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `hackathon`.`user` (
  ...
  `additionalInformation` LONGTEXT NULL DEFAULT NULL,
...
CREATE TABLE IF NOT EXISTS `hackathon`.`training_session` (
  ...
  CONSTRAINT `fk_training_session_user1`
    ...
    REFERENCES `hackathon`.`user` (`user_id`))
ENGINE = InnoDB
DEFAULT CHARACTER SET = utf8mb3;
